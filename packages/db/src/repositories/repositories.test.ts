...
describe("repository layer", () => {
  ...
  it("round-trips source, inbox, and filter data", async () => {
    ...
    try {
      ...
      expect(inboxByTicker[0]?.enrichment?.summary).toBe("The item mentions Acme.");
      ...
      const inboxByReadState = await items.listInboxItems({ readState: "read" });
      expect(inboxByReadState).toHaveLength(1);
      expect(inboxByReadState[0]?.item.id).toBe("item-2");

      const inboxBySource = await items.listInboxItems({ sourceId: "hacker-news" });
  ...
  it("supports enrichment snapshots and ticker corrections", async () => {
    ...
    try {
      ...
      const foundCorrection = await corrections.findEnabledCorrection("Acme Corp");
