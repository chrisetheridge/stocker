      ...
      <CardBody className="space-y-3">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-xs text-slate-400">
              {snapshot.companyName ?? "Market snapshot"}
            </p>
          </div>
          <div className="flex flex-wrap items-center gap-2">
            {stale ? (
              <Badge tone="warning">Stale cache</Badge>
            ) : (
        ...
        <div className="grid gap-3 text-sm sm:grid-cols-2">
          ...
          <div className="rounded-xl bg-black/20 p-3">
            ...
            <p className="mt-1 font-medium text-slate-50">
              {formatSignedNumber(snapshot.dailyChange)} (
              {formatSignedNumber(snapshot.dailyChangePercent)}%)
            </p>
          </div>
          <div className="rounded-xl bg-black/20 p-3">
            <p className="text-xs tracking-[0.2em] text-slate-400 uppercase">
            ...
            <p className="mt-1 font-medium text-slate-50">
              {snapshot.marketCap
                ? snapshot.marketCap.toLocaleString("en-US")
                : "n/a"}
            </p>
          </div>
          <div className="rounded-xl bg-black/20 p-3">
